...
import { sanitizeHtml, containsXssPatterns } from '@/lib/security/xss-protection';
import { maskLeaderboardName } from '@/lib/utils/privacy';
import { advancedFileScan } from '@/lib/security/advanced-file-scanner';
import { canAttemptConsentVerification, recordConsentAttempt } from '@/lib/security/consent-rate-limiter';
...
describe('Phase 1: Critical Security Fixes', () => {
  ...
  describe('Parental Consent Rate Limiting', () => {
    ...
    it('should allow first 5 attempts', async () => {
      ...
      for (let i = 1; i <= 5; i++) {
        const result = await canAttemptConsentVerification(testCode, '127.0.0.1');
    ...
    it('should block after 5 failed attempts', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        await recordConsentAttempt(blockedCode, false, '127.0.0.1', 'test-agent');
      }

      // 6th attempt should be blocked
      const result = await canAttemptConsentVerification(blockedCode, '127.0.0.1');
    ...
    it('should provide retry time on lockout', async () => {
      ...
      // Trigger lockout
      for (let i = 0; i < 5; i++) {
        await recordConsentAttempt(lockedCode, false, '127.0.0.1', 'test-agent');
      }

      const result = await canAttemptConsentVerification(lockedCode, '127.0.0.1');
  ...
  describe('Advanced File Security Scanner', () => {
    it('should detect embedded executables (heuristic)', async () => {
      ...
      const result = await advancedFileScan(exeBuffer, 'test.jpg', 'image/jpeg');
    ...
    it('should detect PHP scripts in images (polyglot)', async () => {
      const phpInImage = Buffer.from('<?php eval($_GET["cmd"]); ?>fake image data');
      
      const result = await advancedFileScan(phpInImage, 'malicious.jpg', 'image/jpeg');
    ...
    it('should detect PDF JavaScript', async () => {
      ...
      const result = await advancedFileScan(pdfWithJs, 'malicious.pdf', 'application/pdf');
    ...
    it('should pass clean image file', async () => {
      ...
      const result = await advancedFileScan(cleanJpeg, 'clean.jpg', 'image/jpeg');
    ...
    it('should block suspicious file extensions', async () => {
      ...
      const result = await advancedFileScan(exeFile, 'malware.exe', 'application/octet-stream');
