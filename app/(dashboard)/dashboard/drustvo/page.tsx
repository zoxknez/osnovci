    ...
    <div className="container max-w-6xl mx-auto p-4 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="space-y-2 text-center md:text-left">
          <h1 className="text-3xl font-bold text-gray-900">Društvo & Stikeri 🤝</h1>
          <p className="text-gray-600">
            Tvoje odeljenje: {student.grade}/{student.class} • {classmates.length} drugara
          </p>
        </div>
        <div className="bg-yellow-50 px-6 py-3 rounded-full border border-yellow-200 flex items-center gap-3">
          <span className="text-2xl">⭐</span>
          <div>
            <p className="text-xs text-yellow-700 font-bold uppercase">Tvoji poeni</p>
            <p className="text-xl font-bold text-yellow-900">{student.gamification?.xp || 0} XP</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-gradient-to-br from-yellow-50 to-orange-50 p-6 rounded-2xl border border-yellow-100 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
            ...
            {[leaderboard[1], leaderboard[0], leaderboard[2]]
              ...
              .map((s) => {
              const rank = s.id === leaderboard[0]?.id ? 1 : s.id === leaderboard[1]?.id ? 2 : 3;
              ...
              return (
                <div 
                  ...
                  className={`bg-white p-4 rounded-xl shadow-sm border-b-4 flex flex-col items-center relative transition-transform hover:-translate-y-1 ${
                    isFirst ? "border-yellow-400 h-48 justify-center z-10 order-2 sm:order-2" : 
                    rank === 2 ? "border-gray-300 h-40 justify-center order-1 sm:order-1" : 
                    "border-orange-300 h-36 justify-center order-3 sm:order-3"
                  }`}
                >
                  <div className={`absolute -top-3 px-3 py-1 rounded-full text-xs font-bold text-white shadow-sm ${
                    isFirst ? "bg-yellow-500" : rank === 2 ? "bg-gray-400" : "bg-orange-400"
                  }`}>
                    {rank}. Mesto
                  </div>
                  
                  <div className={`${isFirst ? "text-5xl mb-3" : "text-4xl mb-2"}`}>
                    {s.avatar || "👤"}
                  </div>
                  
                  <p className={`font-bold text-gray-900 text-center ${isFirst ? "text-lg" : "text-base"}`}>
                    {s.name}
                  </p>
        ...
        {classmates.length === 0 ? (
          <Card className="bg-blue-50 border-blue-200">
            <CardContent className="p-8 text-center flex flex-col items-center gap-4">
              ...
              <div>
                <h3 className="text-xl font-bold text-blue-900">Tvoje odeljenje je još uvek prazno!</h3>
                <p className="text-blue-700 mt-2 max-w-md mx-auto">
                  Izgleda da se tvoji drugari još nisu registrovali. Reci im da prilikom registracije izaberu:
                </p>
                <div className="mt-4 bg-white/50 p-4 rounded-lg inline-block text-left">
                  <p className="text-sm font-semibold text-blue-800">🏫 Škola: {student.school}</p>
                  <p className="text-sm font-semibold text-blue-800">📚 Razred: {student.grade}</p>
                  <p className="text-sm font-semibold text-blue-800">📝 Odeljenje: {student.class}</p>
                </div>
              </div>
              <p className="text-sm text-blue-600 mt-2">
                Kada se registruju, pojaviće se ovde i moći ćeš da im šalješ stikere!
              </p>
            </CardContent>
          </Card>
        ) : (
        ...
        <div className="space-y-4 pt-8 border-t">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {receivedStickers.map((log) => (
              <div key={log.id} className="bg-white p-4 rounded-lg border shadow-sm flex items-center gap-4">
                <div className="text-4xl">{log.sticker.icon}</div>
                <div>
                  <p className="font-bold text-gray-800">{log.sticker.name}</p>
                  <p className="text-sm text-gray-500">
                    Od: <span className="font-medium text-blue-600">{log.sender.name}</span>
                  </p>
