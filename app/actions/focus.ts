...
import { endFocusSession, startFocusSession } from "@/lib/focus/focus-service";
...
export async function endFocusSessionAction(
  ...
  try {
    ...
    const focusStatus =
      status === "COMPLETED" ? FocusStatus.COMPLETED : FocusStatus.ABORTED;

    const result = await endFocusSession(sessionId, student.id, focusStatus);
    ...
    return {
      error:
        error instanceof Error ? error.message : "Failed to end focus session",
    };
  }
}
