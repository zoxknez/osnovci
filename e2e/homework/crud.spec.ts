...
test.describe('Homework CRUD', () => {
  ...
  test('should open create homework modal', async ({ page }) => {
    ...
    // Modal should appear
    await expect(page.locator('text=/novi zadatak|dodaj zadatak/i')).toBeVisible();
  ...
  test('should create new homework', async ({ page }) => {
    ...
    const subjectSelect = page.locator('select[name="subjectId"], [role="combobox"]').first();
    ...
    // Submit
    await page.click('button[type="submit"]:has-text("Sačuvaj"), button:has-text("Dodaj")');
    
    // Should show success message
    await expect(page.locator('text=/uspešno|sačuvano|added/i')).toBeVisible({ timeout: 5000 });
    
    // Should see new homework in list
    await expect(page.locator(`text=Test Zadatak ${timestamp}`)).toBeVisible({ timeout: 3000 });
  });
  ...
  test('should filter homework by status', async ({ page }) => {
    ...
    const filterButton = page.locator('button:has-text("Svi"), select[name="status"]').first();
    ...
    if (await filterButton.isVisible()) {
      ...
      const items = await page.locator('[data-status], [class*="homework"]').all();
  ...
  test('should search homework', async ({ page }) => {
    ...
    // Find search input
    const searchInput = page.locator('input[type="search"], input[placeholder*="pretraž" i]').first();
  ...
  test('should mark homework as done', async ({ page }) => {
    ...
    await page.waitForSelector('[data-homework], [class*="homework-card"]', { timeout: 5000 });
    
    // Find first homework item
    const firstHomework = page.locator('[data-homework], [class*="homework-card"]').first();
    
    if (await firstHomework.isVisible()) {
      // Find and click "Done" button or checkbox
      const doneButton = firstHomework.locator('button:has-text("Gotovo"), input[type="checkbox"], button[aria-label*="završi" i]').first();
      
      if (await doneButton.isVisible()) {
        await doneButton.click();
        
        // Should show success message
        await expect(page.locator('text=/završeno|gotovo|done/i')).toBeVisible({ timeout: 3000 });
      }
    }
  });
  ...
  test('should delete homework', async ({ page }) => {
    ...
    await expect(page.locator(`text=Delete Test ${timestamp}`)).toBeVisible({ timeout: 3000 });
    
    // Find and click delete button
    const homeworkCard = page.locator(`text=Delete Test ${timestamp}`).locator('..').locator('..');
    const deleteButton = homeworkCard.locator('button[aria-label*="obriši" i], button:has-text("Obriši")').first();
    ...
    if (await deleteButton.isVisible()) {
      ...
      await expect(page.locator(`text=Delete Test ${timestamp}`)).not.toBeVisible({ timeout: 3000 });
    }
  });
});

test.describe('Homework Offline Support', () => {
  test('should save homework offline when offline', async ({ page, context }) => {
    await login(page);
    await page.goto('/dashboard/domaci');
    ...
    await expect(page.locator('text=/offline|sačuvano offline|bez interneta/i')).toBeVisible({ timeout: 5000 });
