...
test.describe("Authentication", () => {
  ...
  test("should show validation errors on empty submit", async ({ page }) => {
    ...
    await expect(page.locator("text=/email.*obavezan/i")).toBeVisible({
      timeout: 2000,
    });
  });
  ...
  test("should login with valid credentials", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/dashboard|domaći|raspored/i"),
    ).toBeVisible();
  });
  ...
  test("should show error on invalid credentials", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/pogrešna|neispravna|nije ispravna/i"),
...
test.describe("Account Lockout", () => {
  test("should lock account after 5 failed attempts", async ({ page }) => {
    ...
    await expect(page.locator("text=/zaključan|locked|15 min/i")).toBeVisible({
      timeout: 5000,
    });
  });
});
...
test.describe("Registration", () => {
  test("should show registration form", async ({ page }) => {
    ...
    await expect(page.locator('input[type="password"]')).toBeVisible();
    await expect(
      page.locator('select[name="role"], input[name="role"]'),
    ).toBeVisible();
  });
  ...
  test("should validate password requirements", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/lozinka.*najmanje|minimum.*karaktera/i"),
    ).toBeVisible();
  });
});
