...
test.describe("File Upload Security", () => {
  ...
  test("should accept valid image files", async ({ page }) => {
    ...
    if (await uploadInput.isVisible()) {
      ...
      await expect(
        page.locator("text=/uspešno|uploaded|preview/i"),
  ...
  test("should reject executable files", async ({ page }) => {
    ...
    if (await uploadInput.isVisible()) {
      ...
      // Should show error
      await expect(
        page.locator("text=/nije dozvoljen|not allowed|invalid file type/i"),
      ).toBeVisible({ timeout: 5000 });
    }
  ...
  test("should reject files that are too large", async ({ page }) => {
    ...
    if (await uploadInput.isVisible()) {
      ...
      await expect(
        page.locator("text=/prevelika|too large|size limit/i"),
      ).toBeVisible({ timeout: 5000 });
    }
  ...
  test("should reject script files", async ({ page }) => {
    ...
    if (await uploadInput.isVisible()) {
      ...
      // Should reject
      await expect(
        page.locator("text=/nije dozvoljen|not allowed|invalid/i"),
      ).toBeVisible({ timeout: 5000 });
    }
  ...
  test("should compress large images", async ({ page }) => {
    ...
    if (await uploadInput.isVisible()) {
      ...
      await expect(
        page.locator("text=/kompresija|compressing|optimizing/i"),
      ).toBeVisible({ timeout: 5000 });
    }
...
test.describe("Camera Upload", () => {
  test("should handle camera permission denial gracefully", async ({
    ...
    const cameraButton = page
      .locator('button:has-text("Kamera"), button[aria-label*="kamera" i]')
      .first();
    ...
    if (await cameraButton.isVisible()) {
      ...
      await expect(
        page.locator("text=/dozvola|permission|camera denied/i"),
      ).toBeVisible({ timeout: 5000 });
  ...
  test("should show camera UI when permission granted", async ({
    page,
    context,
  }) => {
    await login(page);
    ...
    if (await cameraButton.isVisible()) {
      ...
      await expect(page.locator("video, canvas, [data-camera]")).toBeVisible({
        timeout: 5000,
      });
    }
  });
