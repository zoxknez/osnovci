...
import { trackAction, trackPageView } from "@/lib/monitoring/sentry-utils";
 ...
 */
export function sendMetric(name: string, data: Record<string, unknown>) {
  const metric: Metric = {
    name,
    value:
      typeof data["value"] === "number" ? data["value"] : JSON.stringify(data),
    timestamp: Date.now(),
    metadata: data,
  };
    ...
    const handleScroll = () => {
      ...
      if (depth > maxDepth) {
        ...
        const nextMilestone = milestones.find(
          (m) => m > lastReported.current && depth >= m,
        );
