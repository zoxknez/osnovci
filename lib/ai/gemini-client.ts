...
function getClient(): GoogleGenerativeAI | null {
  if (!process.env["GEMINI_API_KEY"]) {
    log.warn(
      "GEMINI_API_KEY is not set - AI Tutor will use fallback responses",
    );
    return null;
  }
...
export async function generateTutorResponse(
  ...
  try {
    ...
    const modelName = request.imageBase64
      ? "gemini-1.5-flash"
      : "gemini-1.5-flash";
    ...
    if (error.message?.includes("SAFETY")) {
      return {
        success: false,
        error:
          "Ups! To pitanje ne mogu da obradim. Pokušaj sa drugačijim pitanjem. 🤔",
      };
    }
    ...
    if (error.message?.includes("quota") || error.message?.includes("rate")) {
      return {
        success: false,
        error:
          "AI Tutor je trenutno zauzet. Sačekaj malo pa pokušaj ponovo. ⏳",
      };
    }
...
function buildSystemPrompt(subject?: string, age?: number): string {
  ...
  const complexity =
    age && age < 10 ? "veoma jednostavno" : "jasno i razumljivo";
