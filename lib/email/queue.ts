...
if (!redisConnection && typeof window === "undefined") {
  log.warn(
    "Redis not configured - email queue disabled (emails will be sent synchronously)",
  );
}
...
export async function queueEmail(
  ...
  if (!emailQueue) {
    ...
    try {
      ...
      await transporter.sendMail({
        from:
          emailData.from || process.env["EMAIL_FROM"] || "noreply@osnovci.app",
        to: emailData.to,
      ...
      log.error("Failed to send email synchronously", {
        error,
        to: emailData.to,
      });
      throw error;
  ...
  const job = await emailQueue.add("send-email", emailData, {
    ...
    priority:
      emailData.priority === "high" ? 1 : emailData.priority === "low" ? 3 : 2,
    removeOnComplete: {
 ...
 */
export async function getFailedEmails(limit = 10): Promise<
  Array<{
    id: string;
    to: string;
    subject: string;
