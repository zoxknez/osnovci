...
export const validationSchemas = {
  ...
  fileSize: (maxBytes: number) =>
    z
      .number()
      .max(maxBytes, `Fajl je prevelik (maksimum ${Math.round(maxBytes / 1024 / 1024)}MB)`),
  ...
  hexColor: z
    .string()
    .regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, 'Boja mora biti u hex formatu (#RRGGBB)'),
};

/**
 * Combine multiple validation schemas
 */
export function combineSchemas<T extends z.ZodRawShape>(schemas: T): z.ZodObject<T> {
