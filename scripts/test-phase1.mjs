...
function assertContains(text, substring, message) {
  if (!text.includes(substring)) {
    throw new Error(`${message || 'Assertion failed'}: expected to contain "${substring}"`);
  }
}

function assertNotContains(text, substring, message) {
  if (text.includes(substring)) {
    throw new Error(`${message || 'Assertion failed'}: expected NOT to contain "${substring}"`);
  }
}
...
(async () => {
  ...
  const { sanitizeHtml, containsXssPatterns } = await import('../lib/security/xss-protection.ts');
  ...
  test('Should remove event handlers', () => {
    const result = sanitizeHtml('<img src=x onerror="alert(\'XSS\')">', 'strict');
    assertNotContains(result, 'onerror', 'Event handler removed');
  });

  test('Should allow safe HTML', () => {
    const result = sanitizeHtml('<p><b>Bold</b> and <i>italic</i></p>', 'strict');
    assertEqual(result, '<p><b>Bold</b> and <i>italic</i></p>', 'Safe HTML preserved');
  });

  test('Should remove javascript: protocol', () => {
    const result = sanitizeHtml('<a href="javascript:alert(\'XSS\')">Click</a>', 'strict');
