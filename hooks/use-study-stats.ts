...
export function useStudyStats(): StudyStatsHook {
  ...
  const [dailyStats, setDailyStats] = useLocalStorage<Record<string, DayStats>>(
    "study_daily_stats",
    {},
  );
  ...
  const [dailyGoalMinutes, setDailyGoalMinutes] = useLocalStorage<number>(
    "study_daily_goal",
    60,
  );
  const [hourlyActivity, setHourlyActivity] = useLocalStorage<
      ...
      setDailyStats((prev) => {
        ...
        if (currentSession.subjectName) {
          subjectBreakdown[currentSession.subjectName] =
            (subjectBreakdown[currentSession.subjectName] || 0) +
            durationMinutes;
        }
